...
// Bob initiates a hased timelock conditional payment to Charlie on chain 2
contract BobHTLC_Chain2 {

    // from address
    address payable public fromBob;
    // to address
    address payable public toCharlie;
    // timeout
    uint256 public timeOut;
    // hashlock
    bytes32 public hashLock;    
    ...
    // contructor
    constructor (address payable _toCharlie, bytes32 _hashLock, uint256 _timeOut) public {
        fromBob = msg.sender;
        toCharlie = _toCharlie;
        hashLock = _hashLock;
        timeOut = now + _timeOut * (1 minutes);
        timelock = true;
    }    
    ...
    // executing the transaction -> Charlie gets the payment
    // if valid secretHash presented
    // if timeout still not reached
    function claim(string memory _secretHash) public {
       require(hashLock == sha256(abi.encodePacked(_secretHash)));
